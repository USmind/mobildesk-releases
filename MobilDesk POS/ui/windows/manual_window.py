...
class ManualWindow(QWidget):
    ...
    def crear_interfaz(self):
        ...
        header_card = QFrame()
        header_card.setObjectName("banner")
        ...
        btn_tour = QPushButton("▶ Ver Introducción Guiada (Tour)")
        btn_tour.setProperty("variant", "ghost")
        btn_tour.setToolTip("Abrir la guía rápida de bienvenida")
        btn_tour.clicked.connect(self.abrir_tour_bienvenida)
        h_layout.addWidget(btn_tour)

        layout.addWidget(header_card)

        # Tabs de Categorías (carga diferida: solo se construye la
        # pestaña visible; las demás se crean al seleccionarlas).
        self.tabs = QTabWidget()

        self._tab_builders = [
            ("Punto de Venta", self._tab_ventas),
            ("Inventario y Precios", self._tab_inventario),
            ("Tasa USD/Bs", self._tab_tasa),
            ("Fiados y Créditos", self._tab_fiados),
            ("Usuarios y Roles", self._tab_usuarios),
            ("Caja y Reportes", self._tab_caja_reportes),
            ("App Móvil", self._tab_movil),
            ("Actualizaciones", self._tab_actualizaciones),
        ]
        self._tab_built = set()
        for titulo, _builder in self._tab_builders:
            self.tabs.addTab(QWidget(), titulo)
        self.tabs.currentChanged.connect(self._cargar_tab_diferida)
        self._cargar_tab_diferida(0)

        layout.addWidget(self.tabs, 1)

    def _cargar_tab_diferida(self, index):
        if index in self._tab_built:
            return
        ...
        lay.setSpacing(0)
        lay.addWidget(self._tab_builders[index][1]())
    ...
    def _crear_card_seccion(self, titulo, descripcion, pasos):
        ...
        if descripcion:
            ...
            d_lbl.setObjectName("pageSubtitle")
        ...
        return self._crear_scroll_tab([
            ...
            self._crear_card_seccion(
                ...
                [
                    ...
                    ("Configurar Desglose", "Pulsa <b>'Configurar Desglose'</b> o directamente <b>'Registrar Venta'</b> para abrir la ventana de pago."),
            ...
            self._crear_card_seccion(
                ...
                [
                    ...
                    ("Eliminar Producto", "Si el cliente se arrepiente de llevar un artículo, selecciónalo en la tabla y pulsa <b>'Eliminar'</b>.")
                ]
            )
        ])
    ...
    def _tab_inventario(self):
        return self._crear_scroll_tab([
            self._crear_card_seccion(
                ...
                [
                    ("Nuevo Producto", "Registra un producto indicando Nombre, Marca, Unidad, Costo en USD y Margen de Ganancia (o Precio de Venta directo). Puedes asignarle una Categoría y un Proveedor (ambos opcionales; elige 'Sin categoría' / 'Sin proveedor' si no aplican)."),
                    ("Modificar", "Edita el precio, nombre o datos de cualquier producto haciendo doble clic en la tabla o pulsando 'Modificar'. Nota: el código del producto ahora también se puede editar."),
                    ...
                    ("Ajuste de Stock", "Para registrar productos vencidos, dañados o cuadres de inventario, utiliza 'Ajuste Stock'. Permite sumar o restar existencias."),
        ...
        return self._crear_scroll_tab([
            self._crear_card_seccion(
                ...
                [
                    ...
                    ("Tasa del BCV en un clic", "Pulsa <b>'Obtener Tasa del BCV'</b> y la tasa oficial queda aplicada al instante, sin pasos extra."),
        ...
        return self._crear_scroll_tab([
            self._crear_card_seccion(
                ...
                [
                    ...
                    ("3. Registrar Abonos / Pagos", "Selecciona el cliente y pulsa el botón verde <b>'Cobrar / Registrar Abono'</b>. Puedes usar los botones rápidos <b>[Pagar Todo (100%)]</b>, <b>[Pagar la Mitad (50%)]</b> o ingresar el monto exacto entregado."),
                    ("4. Historial de Abonos", "Pulsa 'Ver Historial' para ver cada pago anterior con fecha y monto.")
                ]
            )
        ])
        ...
        return self._crear_scroll_tab([
            self._crear_card_seccion(
                ...
                [
                    ...
                    ("Editar Usuario", "Selecciona un usuario en la tabla para modificar su nombre, usuario de login, rol o cambiar su contraseña."),
                    ("Eliminar Usuario", "Elimina cuentas que ya no trabajen en el negocio con confirmación de seguridad. El sistema protege al último Administrador para que nunca quede bloqueado.")
                ]
            )
        ])
