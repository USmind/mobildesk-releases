...
# Generate Pixel-Perfect Native Mobile App UI with generous spacing
def create_native_mobile_app_image(w=580, h=920):
    ...
    p.setPen(Qt.NoPen)
    p.drawRect(0, 0, w, 115)

    # Title
    p.setFont(QFont("Segoe UI", 18, QFont.Bold))
    p.setPen(QColor("#ffffff"))
    p.drawText(QRectF(20, 42, w - 40, 32), Qt.AlignLeft | Qt.AlignVCenter, "📱 MobilDesk App")

    # Rate
    p.setFont(QFont("Segoe UI", 12, QFont.Bold))
    p.setPen(QColor("#fbbf24"))
    p.drawText(QRectF(20, 78, w - 40, 26), Qt.AlignLeft, "Tasa BCV: 1 USD = 900.00 Bs")

    # Search Bar
    p.setBrush(QBrush(QColor("#ffffff")))
    p.setPen(QPen(QColor("#cbd5e1"), 1.5))
    p.drawRoundedRect(QRectF(18, 130, w - 36, 44), 10, 10)
    p.setFont(QFont("Segoe UI", 13, QFont.Normal))
    p.setPen(QColor("#64748b"))
    p.drawText(QRectF(34, 130, w - 68, 44), Qt.AlignLeft | Qt.AlignVCenter, "🔍 Buscar producto o escanear...")

    # Product Cards
    products = [
        ("Harina PAN 1kg", "7591058001000", "$1.20 USD", "Bs 1,080.00", "Stock: 40", "#15803d", "#dcfce7"),
        ("Arroz Primor 1kg", "7591058002001", "$1.10 USD", "Bs 990.00", "Stock: 35", "#15803d", "#dcfce7"),
        ("Aceite Vegetal Vatel 1L", "7591058003002", "$2.40 USD", "Bs 2,160.00", "Stock: 25", "#15803d", "#dcfce7"),
        ("Café Fama de América 250g", "7591058005004", "$2.80 USD", "Bs 2,520.00", "Stock: 20", "#15803d", "#dcfce7"),
        ("Mantequilla Mavesa 500g", "7591058007006", "$2.20 USD", "Bs 1,980.00", "Stock: 18", "#15803d", "#dcfce7"),
        ("Leche en Polvo 1kg", "7591058008007", "$8.50 USD", "Bs 7,650.00", "Stock: 15", "#15803d", "#dcfce7"),
    ]
    ...
    for name, code, usd, bs, stock, st_color, st_bg in products:
        ...
        p.setPen(QColor("#64748b"))
        p.drawText(QRectF(32, card_y + 38, w - 160, 20), Qt.AlignLeft, f"Cód: {code}")

        # Prices
        p.setFont(QFont("Segoe UI", 15, QFont.Black))
        p.setPen(QColor("#1e3a8a"))
        p.drawText(QRectF(32, card_y + 64, 130, 26), Qt.AlignLeft, usd)

        p.setFont(QFont("Segoe UI", 13, QFont.Bold))
        p.setPen(QColor("#15803d"))
        p.drawText(QRectF(155, card_y + 66, 170, 24), Qt.AlignLeft, bs)

        # Stock Pill Badge
        p.setBrush(QBrush(QColor(st_bg)))
        p.setPen(Qt.NoPen)
        p.drawRoundedRect(QRectF(w - 145, card_y + 34, 110, 32), 16, 16)
        p.setFont(QFont("Segoe UI", 12, QFont.Bold))
        p.setPen(QColor(st_color))
        p.drawText(QRectF(w - 145, card_y + 34, 110, 32), Qt.AlignCenter, stock)
...
def create_base_canvas(width=W, height=H):
    ...
    # Ambient Glow
    rad = QRadialGradient(width / 2, 190, 520)
...
def draw_top_badge(p, text, color="#38bdf8", y=105, width=W):
    w = 880
    h = 58
    x = (width - w) / 2
...
def draw_header_title(p, title, y=180, h=155, size=42, width=W):
    p.setFont(QFont("Segoe UI", size, QFont.Black))
    p.setPen(QColor("#ffffff"))
    p.drawText(QRectF(50, y, width - 100, h), Qt.AlignCenter | Qt.TextWordWrap, title)

def draw_pc_screen(p, path, x=60, y=355, w=960, h=930, title="💻 MobilDesk POS · Software para PC"):
    p.save()
    ...
    p.drawText(QRectF(x + 85, y + 10, w - 100, 28), Qt.AlignLeft | Qt.AlignVCenter, title)
...
def draw_smartphone(p, qimg_src, phone_x=230, phone_y=355, phone_w=620, phone_h=930):
    p.save()
...
def draw_bottom_highlight(p, main_text, sub_text="", y=1325):
    box_w = 960
    box_h = 175
    box_x = (W - box_w) / 2
    ...
    p.setPen(QColor("#fbbf24"))
    p.drawText(QRectF(box_x + 24, y + 20, box_w - 48, 40), Qt.AlignCenter, main_text)

    if sub_text:
        p.setFont(QFont("Segoe UI", 20, QFont.Medium))
        p.setPen(QColor("#e2e8f0"))
        p.drawText(QRectF(box_x + 24, y + 68, box_w - 48, 90), Qt.AlignCenter | Qt.TextWordWrap, sub_text)
...
def draw_footer_cta(p, slide_num, total_slides=6, cta_text="👉 Desliza para ver más ➡️"):
    ...
    p.drawText(QRectF(0, 1530, W, 42), Qt.AlignCenter, cta_text)

    p.setFont(QFont("Segoe UI", 18, QFont.DemiBold))
    p.setPen(QColor("#64748b"))
    p.drawText(QRectF(0, 1580, W, 32), Qt.AlignCenter, f"Slide {slide_num} de {total_slides} · MobilDesk POS")
...
draw_top_badge(pf, "💎 SISTEMA DE VENTAS POS & APP MÓVIL 🇻🇪", "#fbbf24", 90)
draw_header_title(pf, "Digitaliza tu negocio hoy mismo sin pagar mensualidades", 160, 125, 42)
...
pf.drawText(QRectF(70, card_y + 20, 940, 36), Qt.AlignCenter, "OFERTA ESTRELLA: LICENCIA VITALICIA")

pf.setFont(QFont("Segoe UI", 85, QFont.Black))
pf.setPen(QColor("#fbbf24"))
pf.drawText(QRectF(70, card_y + 56, 940, 95), Qt.AlignCenter, "$15 USD")

pf.setFont(QFont("Segoe UI", 22, QFont.Bold))
pf.setPen(QColor("#ffffff"))
pf.drawText(QRectF(70, card_y + 158, 940, 34), Qt.AlignCenter, "Pago Único Permanente a Tasa BCV · Cero Mensualidades")

pf.setPen(QPen(QColor("#1e3a8a"), 2))
pf.drawLine(130, card_y + 204, 950, card_y + 204)

def draw_offer_line(p, text, y_pos):
    p.setFont(QFont("Segoe UI", 21, QFont.Bold))
    p.setPen(QColor("#4ade80"))
    p.drawText(QRectF(110, y_pos, 35, 32), Qt.AlignLeft | Qt.AlignVCenter, "✔")
    p.setFont(QFont("Segoe UI", 20, QFont.DemiBold))
    p.setPen(QColor("#f8fafc"))
    p.drawText(QRectF(155, y_pos, 810, 32), Qt.AlignLeft | Qt.AlignVCenter, text)

draw_offer_line(pf, "Software Punto de Venta para PC (Windows)", card_y + 225)
draw_offer_line(pf, "App Móvil sincronizada para teléfono Android", card_y + 278)
draw_offer_line(pf, "Control de Inventario, Ventas, Caja y Fiados", card_y + 331)
draw_offer_line(pf, "Conversor Automático de Tasa BCV (USD / Bs)", card_y + 384)
draw_offer_line(pf, "Funciona 100% rápido aunque no tengas internet", card_y + 437)
draw_offer_line(pf, "Instalación en 1 Clic + Video Tutorial incluido", card_y + 490)
draw_offer_line(pf, "Soporte Técnico y Garantía Total de funcionamiento", card_y + 543)
draw_offer_line(pf, "Prueba DEMO Gratis disponible para probarlo hoy", card_y + 596)

plans_y = card_y + 655
pf.setBrush(QBrush(QColor("#081226")))
pf.setPen(QPen(QColor("#38bdf8"), 2))
pf.drawRoundedRect(QRectF(110, plans_y, 860, 115), 16, 16)
...
pf.setFont(QFont("Segoe UI", 18, QFont.Medium))
pf.setPen(QColor("#cbd5e1"))
pf.drawText(QRectF(130, plans_y + 56, 820, 36), Qt.AlignCenter, "Plan Anual: $12 / año   •   Plan Mensual: $5 / mes   •   Vitalicio: $15 único")

cta_y = 1150
pf.setBrush(QBrush(QColor("#2563eb")))
pf.setPen(QPen(QColor("#93c5fd"), 3))
pf.drawRoundedRect(QRectF(70, cta_y, 940, 168), 22, 22)

pf.setFont(QFont("Segoe UI", 24, QFont.Bold))
pf.setPen(QColor("#ffffff"))
pf.drawText(QRectF(70, cta_y + 20, 940, 32), Qt.AlignCenter, "📩 ESCRÍBENOS PARA PEDIR TU DEMO GRATIS:")

pf.setFont(QFont("Segoe UI", 40, QFont.Black))
pf.setPen(QColor("#fbbf24"))
pf.drawText(QRectF(70, cta_y + 64, 940, 52), Qt.AlignCenter, "mobildeskpos@gmail.com")

pf.setFont(QFont("Segoe UI", 18, QFont.DemiBold))
pf.setPen(QColor("#e0e7ff"))
pf.drawText(QRectF(70, cta_y + 124, 940, 26), Qt.AlignCenter, "Te enviamos el instalador y la app móvil de inmediato")
...
draw_top_badge(p1, "🏪 PARA DUEÑOS DE BODEGAS EN VENEZUELA 🇻🇪", "#38bdf8", 105)
draw_header_title(p1, "Deja de anotar las ventas y los fiados en un cuaderno 📖❌", 180)
draw_pc_screen(p1, img_sales_real, 60, 355, 960, 930, "🛒 Facturación y Punto de Venta en Vivo · MobilDesk")
...
draw_top_badge(p2, "1. FACTURACIÓN & TASA BCV AUTOMÁTICA", "#38bdf8", 105)
draw_header_title(p2, "Cobra en $ y calcula el vuelto exacto en Bolívares 💵", 180)
draw_pc_screen(p2, img_sales_real, 60, 355, 960, 930, "🛒 Facturación y Punto de Venta en Vivo · MobilDesk")
draw_bottom_highlight(
    p2,
...
draw_top_badge(p3, "2. INVENTARIO Y GESTIÓN DE PRODUCTOS", "#38bdf8", 105)
draw_header_title(p3, "Controla existencias, precios en $ y alertas de stock 📦", 180)
draw_pc_screen(p3, img_inv_real, 60, 355, 960, 930, "📦 Catálogo de Productos e Inventario · MobilDesk")
draw_bottom_highlight(
    p3,
...
draw_top_badge(p4, "3. CONTROL DE FIADOS Y DEUDAS", "#fbbf24", 105)
draw_header_title(p4, "Olvídate del cuaderno de fiados y recupera tu dinero 📋✨", 180)
draw_pc_screen(p4, img_fiados_real, 60, 355, 960, 930, "👥 Cuentas por Cobrar (Fiados) · MobilDesk")
draw_bottom_highlight(
    p4,
...
draw_top_badge(p5, "4. APP MÓVIL EN TU TELÉFONO", "#38bdf8", 105)
draw_header_title(p5, "Tu bodega en tu bolsillo sincronizada en tiempo real 📱", 180)
draw_smartphone(p5, img_mobile_fresh, 230, 355, 620, 930)
draw_bottom_highlight(
    p5,
...
draw_top_badge(p6, "💎 OFERTA ESPECIAL · PAGO ÚNICO", "#fbbf24", 105)
draw_header_title(p6, "Digitaliza tu negocio hoy por solo $15 USD 🚀", 180)
...
p6.drawText(QRectF(70, card_y6 + 24, 940, 36), Qt.AlignCenter, "LICENCIA PERMANENTE VITALICIA")

p6.setFont(QFont("Segoe UI", 85, QFont.Black))
p6.setPen(QColor("#fbbf24"))
p6.drawText(QRectF(70, card_y6 + 64, 940, 95), Qt.AlignCenter, "$15 USD")
...
def draw_chk6(text, y_pos):
    ...
    p6.drawText(QRectF(120, y_pos, 35, 32), Qt.AlignLeft | Qt.AlignVCenter, "✔")
    p6.setFont(QFont("Segoe UI", 21, QFont.DemiBold))
    p6.setPen(QColor("#f8fafc"))
    p6.drawText(QRectF(165, y_pos, 770, 32), Qt.AlignLeft | Qt.AlignVCenter, text)
...
p6.drawRoundedRect(QRectF(70, cta_y6, 940, 165), 22, 22)

p6.setFont(QFont("Segoe UI", 24, QFont.Bold))
p6.setPen(QColor("#ffffff"))
p6.drawText(QRectF(70, cta_y6 + 20, 940, 32), Qt.AlignCenter, "📩 ESCRÍBENOS PARA PEDIR TU DEMO:")

p6.setFont(QFont("Segoe UI", 40, QFont.Black))
p6.setPen(QColor("#fbbf24"))
p6.drawText(QRectF(70, cta_y6 + 64, 940, 52), Qt.AlignCenter, "mobildeskpos@gmail.com")

p6.setFont(QFont("Segoe UI", 18, QFont.DemiBold))
p6.setPen(QColor("#e0e7ff"))
p6.drawText(QRectF(70, cta_y6 + 122, 940, 26), Qt.AlignCenter, "Te respondemos de inmediato con el instalador")
...
draw_top_badge(pb, "🏪 SISTEMA DE VENTAS · MOBILDESK POS 🇻🇪", "#fbbf24", 45, BW)
draw_header_title(pb, "El Punto de Venta Completo para tu Bodega o Negocio", 120, 95, 36, BW)
...
pb.drawText(QRectF(75, card_b_y + 30, 365, 30), Qt.AlignCenter, "LICENCIA PERMANENTE")

pb.setFont(QFont("Segoe UI", 70, QFont.Black))
pb.setPen(QColor("#fbbf24"))
pb.drawText(QRectF(75, card_b_y + 65, 365, 80), Qt.AlignCenter, "$15")

pb.setFont(QFont("Segoe UI", 17, QFont.Bold))
pb.setPen(QColor("#ffffff"))
pb.drawText(QRectF(75, card_b_y + 155, 365, 26), Qt.AlignCenter, "PAGO ÚNICO · TASA BCV")

pb.setFont(QFont("Segoe UI", 15, QFont.Medium))
pb.setPen(QColor("#94a3b8"))
pb.drawText(QRectF(75, card_b_y + 188, 365, 24), Qt.AlignCenter, "Sin Rentas Mensuales")

pb.setPen(QPen(QColor("#1e3a8a"), 2))
pb.drawLine(465, card_b_y + 30, 465, card_b_y + 395)

def draw_b_chk(text, y_pos):
    pb.setFont(QFont("Segoe UI", 19, QFont.Bold))
    pb.setPen(QColor("#4ade80"))
    pb.drawText(QRectF(495, y_pos, 30, 30), Qt.AlignLeft | Qt.AlignVCenter, "✔")
    pb.setFont(QFont("Segoe UI", 18, QFont.DemiBold))
    pb.setPen(QColor("#f8fafc"))
    pb.drawText(QRectF(535, y_pos, 475, 30), Qt.AlignLeft | Qt.AlignVCenter, text)

draw_b_chk("Punto de Venta para PC (Windows)", card_b_y + 35)
draw_b_chk("App Móvil Android sincronizada", card_b_y + 88)
draw_b_chk("Tasa BCV Automática ($ y Bs)", card_b_y + 141)
draw_b_chk("Control de Fiados y Clientes", card_b_y + 194)
draw_b_chk("Inventario y Código de Barras", card_b_y + 247)
draw_b_chk("Cierre de Caja Diario y Reportes", card_b_y + 300)
draw_b_chk("Instalación rápida + Soporte", card_b_y + 353)

cta_by = card_b_y + 425
pb.setBrush(QBrush(QColor("#2563eb")))
pb.setPen(QPen(QColor("#93c5fd"), 2.5))
pb.drawRoundedRect(QRectF(80, cta_by, 920, 130), 18, 18)
...
pb.drawText(QRectF(80, cta_by + 98, 920, 24), Qt.AlignCenter, "Instalación inmediata • Soporte y Garantía Total")

pb.setFont(QFont("Segoe UI", 20, QFont.Bold))
pb.setPen(QColor("#38bdf8"))
pb.drawText(QRectF(0, 855, BW, 35), Qt.AlignCenter, "👉 Comenta 'DEMO' o Escríbenos para enviarte el instalador")

pb.setFont(QFont("Segoe UI", 15, QFont.DemiBold))
pb.setPen(QColor("#64748b"))
pb.drawText(QRectF(0, 900, BW, 25), Qt.AlignCenter, "MobilDesk POS · Hecho para Comercios y Bodegas en Venezuela 🇻🇪")

pb.end()
img_b.save(str(out_dir / "Banner_Cuadrado_1080x1080.png"), "PNG")

print("PERFECT_SPACING_FIXED_100_OK")
